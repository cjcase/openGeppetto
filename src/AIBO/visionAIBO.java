...
import openGeppetto.UDPListener;
import openGeppetto.VisionListener;

// Modified from Tekkotsu by cjcase
public class visionAIBO extends UDPListener implements VisionListener {
    ...
    public visionAIBO() { super(); }
	public visionAIBO(int port) { super(port); }
	public visionAIBO(String host, int port) { super(host,port); }
	...
	public boolean readColorModel(InputStream in) throws java.io.IOException {
		...
		// (btw, java is stupid) [cjcase: lol, true that]
	...
	static int pixelYUV2RGB(int y, int u, int v) {
		u=u*2-255;
		v=v-128;
		int b=y+u;
		int r=y+v;
	...
	public void close() {
		if(mysock==null || mysock.isClosed())
			return;
		destroy=true;
		byte[] auxBuf = ("refreshSensors\n").getBytes();
		try {
			mysock.send(new DatagramPacket(auxBuf,auxBuf.length));
